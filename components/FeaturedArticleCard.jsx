...
const FeaturedArticleCard = ({ article }) => (
  ...
  <div className="relative h-72">
    ...
    <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full">
      <p className="text-white mb-4 text-shadow font-semibold text-xs">{moment(article.createdAt).format('DD MMM YYYY')}</p>
      <p className="text-white mb-4 text-shadow font-semibold text-2xl text-center">{article.title}</p>
      <div className="flex items-center absolute bottom-5 w-full justify-center">
        ...
        />
        <p className="inline align-middle text-white text-shadow ml-2 font-medium">{article.author.name}</p>
      </div>
    </div>
