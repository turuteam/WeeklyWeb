...
const FeaturedArticleCard = ({ article }) => (
  ...
  <div className="relative h-72">
    ...
    <div className="absolute rounded-lg bg-center bg-gradient-to-b opacity-80 from-sky-500 via-gray-700 to-black w-full h-72" />
    <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full">
      <p className="text-white mb-4 text-shadow font-semibold text-xs">{moment(article.updatedAt).format('D MMMM YYYY')}</p>
      <p className="text-white mb-4 text-shadow font-semibold text-sm text-center">{article.title}</p>
